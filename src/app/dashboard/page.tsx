...
export default function DashboardPage() {
    ...
    const copyInstallCommand = (profile: Profile) => {
        const baseUrl = window.location.origin;
        const params = new URLSearchParams({
            name: profile.name,
            stack: profile.stack.join(',')
        });
        const command = `irm "${baseUrl}/api/setup?${params.toString()}" | iex`;
                                    ...
                                    <div key={profile.id} className="card profile-card">
                                        ...
                                        <div className="profile-content">
                                            ...
                                            <div className="profile-actions" style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                                ...
                                                <Button variant="secondary" onClick={() => copyInstallCommand(profile)}>
                                                    {copiedId === profile.id ? 'Copied!' : 'Copy Command'}
                                                </Button>
                                            </div>
                                        </div>
                                    </div>
